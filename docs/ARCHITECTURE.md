...
实现前需要通过技术验证并形成独立 ADR：

1. **ADR-001：Rust 与 Python 的 stdio NDJSON RPC**——吞吐、取消、崩溃和日志隔离。
2. **ADR-002：Python sidecar 打包方案**——PyInstaller、独立运行时和签名兼容性。
3. **ADR-003：项目 SQLite 唯一写入者**——连接、WAL、迁移与后台 worker。
4. **ADR-004：插件隔离级别**——独立进程、文件范围和环境变量注入。
5. **ADR-005：全局资产导入策略**——硬链接、复制和外置磁盘行为。
6. **ADR-006：Grok 媒体能力适配**——命令、输出发现、额度和失败语义。
7. **ADR-007：本地模型组件管理**——下载、校验、版本共存和卸载确认。
8. **ADR-008：FFmpeg 时间线编译器**——预览与最终渲染一致性。
